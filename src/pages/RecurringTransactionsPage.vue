        ...
        <div class="page-header q-pa-md">
            <div class="row items-center justify-between">
                <div>
                    ...
                    <p class="text-subtitle1 text-grey-6 q-ma-none">Manage your recurring income and expenses</p>
                </div>
                <div class="row q-gutter-sm">
                    <q-btn color="primary" icon="add" label="Add Recurring" @click="showAddRecurringDialog" />
                    <q-btn color="secondary" icon="refresh" label="Process Now" @click="processRecurringTransactions" />
                </div>
            </div>
        </div>
                    ...
                    <div v-else>
                        <q-list separator>
                            <q-item v-for="recurring in activeRecurringTransactions" :key="recurring.id"
                                ...
                                <q-item-section>
                                    <q-item-label class="text-weight-bold">{{ recurring.description }}</q-item-label>
                                    <q-item-label caption>
            ...
            <q-card style="min-width: 500px">
                ...
                <q-card-section>
                    <q-form @submit="saveRecurringTransaction" class="q-gutter-md">
                        ...
                        <q-input v-model.number="recurringForm.amount" label="Amount" type="number" step="0.01" outlined
                            :rules="[val => val > 0 || 'Amount must be greater than 0']" />
                        <q-select v-model="recurringForm.type" :options="typeOptions" label="Type" outlined emit-value
                            map-options :rules="[val => !!val || 'Type is required']" />
...
// Computed
const { recurringTransactions, categories, upcomingBills, overdueBills } = financialStore

const activeRecurringTransactions = computed(() => {
    return recurringTransactions.filter(rt => rt.isActive !== false)
})
